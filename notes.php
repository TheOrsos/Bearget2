...
require_once 'friends_functions.php'; // Aggiunto per le funzioni degli amici
...
<body class="text-gray-300">
        ...
        <main class="flex-1 p-6 lg:p-10 overflow-y-auto">
            <header class="flex flex-wrap justify-between items-center gap-4 mb-8">
                ...
                <div class="flex items-center gap-2">
                    <button id="toggle-filter-btn" class="bg-gray-600 hover:bg-gray-500 text-white font-semibold py-2 px-5 rounded-lg flex items-center transition-colors">
    ...
    <!-- MODALE CONDIVISIONE NOTA -->
    <div id="share-note-modal" class="fixed inset-0 z-50 flex items-center justify-center p-4 hidden">
        <div class="fixed inset-0 bg-black bg-opacity-60 opacity-0 modal-backdrop" onclick="closeModal('share-note-modal')"></div>
        <div class="bg-gray-800 rounded-2xl shadow-xl w-full max-w-md p-6 transform scale-95 opacity-0 modal-content">
            <h3 class="text-2xl font-bold text-white mb-4">Condividi Nota</h3>
            <p class="text-gray-400 mb-6">Invita un amico a collaborare a questa nota inserendo il suo codice amico.</p>
            <form id="share-note-form">
                ...
                <div class="mb-4">
                    ...
                    <div class="flex gap-2">
                        <input type="text" name="friend_code" id="friend_code" class="flex-grow bg-gray-700 text-white rounded-lg px-3 py-2" required>
                        <button type="button" id="choose-friend-btn-note" class="bg-gray-600 hover:bg-gray-500 text-white font-semibold py-2 px-4 rounded-lg">Scegli</button>
                    </div>
                </div>
                <div class="mb-6">
                    <label for="permission" class="block text-sm font-medium text-gray-300 mb-1">Permesso</label>
                    <select name="permission" id="permission" class="w-full bg-gray-700 text-white rounded-lg px-3 py-2">
                        <option value="edit">Può modificare</option>
                        <option value="view">Può solo visualizzare</option>
                    </select>
                </div>
                <div class="flex justify-end gap-4">
                    <button type="button" class="text-gray-400 hover:text-white" onclick="closeModal('share-note-modal')">Annulla</button>
                    <button type="submit" class="bg-primary-600 hover:bg-primary-700 text-white font-semibold py-2 px-5 rounded-lg">Condividi</button>
                </div>
            </form>
        </div>
    </div>

    <!-- NUOVO MODALE LISTA AMICI -->
    <div id="friends-list-modal-note" class="fixed inset-0 z-50 flex items-center justify-center p-4 hidden">
        <div class="fixed inset-0 bg-black bg-opacity-60 opacity-0 modal-backdrop" onclick="closeModal('friends-list-modal-note')"></div>
        <div class="bg-gray-800 rounded-2xl shadow-xl w-full max-w-md p-6 transform scale-95 opacity-0 modal-content">
            <h3 class="text-2xl font-bold text-white mb-4">Scegli un Amico</h3>
            <div class="mb-4">
                <input type="text" id="friend-search-input-note" placeholder="Cerca per nome utente..." class="w-full bg-gray-700 text-white rounded-lg px-3 py-2 text-sm border border-gray-600 focus:ring-primary-500 focus:border-primary-500">
            </div>
            <div id="friends-list-container-note" class="space-y-2 max-h-60 overflow-y-auto">
                <!-- L'elenco degli amici verrà inserito qui dal JS -->
            </div>
            <div class="flex justify-end mt-6">
                <button type="button" class="text-gray-400 hover:text-white" onclick="closeModal('friends-list-modal-note')">Chiudi</button>
            </div>
        </div>
    </div>
...
function setupFriendSelector(prefix) {
                    ...
                    .then(data => {
                        if (data.success) {
                            ...
                            if (typeof showToast === 'function') {
                                showToast('Errore nel caricare la lista amici.', 'danger');
                            } else {
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            const filtersAreActive = urlParams.has('text_search') && urlParams.get('text_search') !== '' ||
                                     urlParams.has('id_search') && urlParams.get('id_search') !== '' ||
                                     urlParams.has('date_search') && urlParams.get('date_search') !== '';

            function setButtonToHide() {
                toggleFilterBtn.innerHTML = `<svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path></svg> <span>Nascondi Filtri</span>`;
                toggleFilterBtn.classList.remove('bg-gray-600', 'hover:bg-gray-500');
                toggleFilterBtn.classList.add('bg-red-600', 'hover:bg-red-500');
            }

            function setButtonToShow() {
                toggleFilterBtn.innerHTML = `<svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 4a1 1 0 011-1h16a1 1 0 011 1v2.586a1 1 0 01-.293.707l-6.414 6.414a1 1 0 00-.293.707V17l-4 4v-6.586a1 1 0 00-.293-.707L3.293 7.293A1 1 0 013 6.586V4z"></path></svg> <span>Mostra Filtri</span>`;
                toggleFilterBtn.classList.remove('bg-red-600', 'hover:bg-red-500');
                toggleFilterBtn.classList.add('bg-gray-600', 'hover:bg-gray-500');
            }
            ...
            // --- NUOVA LOGICA PER LA SELEZIONE AMICO ---
            setupFriendSelector('note');
        ...
        function updateNoteInGrid(note) {
            ...
            if (card) {
                ...
                if (note.creator_id != CURRENT_USER_ID) {
                    tagsHTML += `<span class="text-xs bg-green-600 text-white font-semibold px-2 py-1 rounded-full shared-tag">Condivisa</span>`;
                }
                const todolistItems = JSON.parse(note.todolist_content || '[]');
                if (Array.isArray(todolistItems) && todolistItems.length > 0) {
                    tagsHTML += `<span class="text-xs bg-primary-600 text-white font-semibold px-2 py-1 rounded-full todo-tag">To-Do</span>`;
                }
