...
switch ($event->type) {
    ...
    case 'customer.subscription.updated':
        ...
        // Check if the subscription is marked to be canceled at the end of the period
        if ($subscription->cancel_at_period_end) {
            $new_status = 'pending_cancellation';
        }

        // Also check if the subscription status is past_due
        if ($subscription->status === 'past_due') {
            $new_status = 'past_due';
        }

        // If the user has reactivated a pending cancellation, the status will be active
        if (!$subscription->cancel_at_period_end && $subscription->status === 'active') {
            $new_status = 'active';
        }

        $sql = "UPDATE users SET subscription_status = ? WHERE stripe_subscription_id = ?";
        $stmt = $conn->prepare($sql);
        $stmt->bind_param("ss", $new_status, $stripe_subscription_id);
        $stmt->execute();
        $stmt->close();
        break;
