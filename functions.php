...
function get_base_url() {
    ...
    // Rimuove il nome del file script per ottenere la directory base
    $base_path = substr($script_name, 0, strrpos($script_name, '/') + 1);
...
function update_note_permission($conn, $note_id, $member_id, $permission, $current_user_id) {
    ...
    if ($success) {
        ...
        return ['success' => false, 'message' => 'Errore durante laggiornamento dei permessi.'];
    }
}
...
/**
 * Ottiene il numero di notifiche non lette di un utente.
 */
 ...
 */
function get_all_notifications($conn, $user_id) {
...
/**
 * Controlla se un utente è membro di un fondo.
 */
function is_fund_member($conn, $fund_id, $user_id) {
    $sql = "SELECT id FROM shared_fund_members WHERE fund_id = ? AND user_id = ?";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("ii", $fund_id, $user_id);
    $stmt->execute();
    $stmt->store_result();
...
/**
 * Ottiene lo stato di una richiesta di amicizia.
 */
function get_friendship_status($conn, $request_id) {
...
/**
 * Ottiene lo stato di un trasferimento di denaro.
 */
function get_money_transfer_status($conn, $transfer_id) {
    $sql = "SELECT status FROM money_transfers WHERE id = ?";
    $stmt = $conn->prepare($sql);
...
function get_fund_contributions($conn, $fund_id) {
    $contributions = [];
    // Aggiungo sfc.id, sfc.user_id, sfc.transaction_id, e t.account_id per le nuove funzionalità
    $sql = "SELECT sfc.id, sfc.user_id, sfc.transaction_id, sfc.amount, sfc.contribution_date, u.username, t.account_id
            FROM shared_fund_contributions sfc
            JOIN users u ON sfc.user_id = u.id
            LEFT JOIN transactions t ON sfc.transaction_id = t.id
            WHERE sfc.fund_id = ?
...
// =============================================================================
// FUNZIONI DI AUTENTICAZIONE E TOKEN
// =============================================================================

/**
 * Genera un token sicuro.
 * @param int $length La lunghezza del token in byte.
 * @return string Il token in formato esadecimale.
 */
...
/**
 * Salva un token di autenticazione a lungo termine per un utente.
 *
 * @param object $conn La connessione al database.
 * @param int $user_id L'ID dell'utente.
 * @return array|null Un array con i dati del token o null in caso di fallimento.
 */
function remember_user_token($conn, $user_id) {
    try {
        $selector = generate_token(12);
        ...
        $stmt = $conn->prepare($sql);
        $stmt->bind_param("ssis", $selector, $hashed_validator, $user_id, $expires_db_format);
        ...
        // Log dell'errore (opzionale, ma consigliato)
        error_log('Errore durante la creazione del token: ' . $e->getMessage());
    }
 ...
 * @param object $conn La connessione al database.
 */
function clear_expired_tokens($conn) {
    $now = date('Y-m-d H:i:s');
    $sql = "DELETE FROM auth_tokens WHERE expires < ?";
    $stmt = $conn->prepare($sql);
...
/**
 * Valida un token di autenticazione dal cookie.
 * @param object $conn La connessione al database.
 * @param string $token Il valore del cookie.
 * @return array|null I dati dell'utente se il token è valido, altrimenti null.
 */
function validate_remember_me_token($conn, $token) {
    ...
    if ($token_data) {
        ...
        if (new DateTime() > new DateTime($token_data['expires'])) {
            // Token scaduto, è buona norma rimuoverlo
            $sql_delete = "DELETE FROM auth_tokens WHERE id = ?";
            $stmt_delete = $conn->prepare($sql_delete);
            $stmt_delete->bind_param("i", $token_data['id']);
            $stmt_delete->execute();
            $stmt_delete->close();
            return null;
        }

        // Confronta il validatore
        if (password_verify($validator, $token_data['hashed_validator'])) {
            // Token valido, restituisci i dati dell'utente
            return get_user_by_id($conn, $token_data['user_id']);
        }
    }
...
/**
 * Esegue il login di un utente, popolando la sessione.
 * @param array $user I dati dell'utente dal database.
 */
