        ...
        <main class="flex-1 p-6 lg:p-10 overflow-y-auto">
            ...
            <div class="space-y-8">
                <div class="bg-gray-800 rounded-2xl p-6">
                    ...
                    <form action="update_theme.php" method="POST">
                        <div class="grid grid-cols-3 sm:grid-cols-6 gap-4">
                            <?php
                            $is_pro = ($user['subscription_status'] === 'active' || $user['subscription_status'] === 'lifetime' || $user['subscription_status'] === 'pending_cancellation');
                            $pro_themes = ['dark-gold']; // Array of PRO themes
                            $themes = [
                                ...
                                'dark-gold' => 'bg-yellow-500',
                                'modern-dark' => 'bg-violet-500',
                                'foggy-gray' => 'bg-gray-400',
                            ];
                            foreach($themes as $theme_key => $color_class):
                                if (in_array($theme_key, $pro_themes) && !$is_pro) {
                            ?>
                                <div class="relative">
                                    <div class="w-16 h-16 <?php echo $color_class; ?> rounded-full mx-auto opacity-50"></div>
                ...
                <div class="bg-gray-800 rounded-2xl p-6 lg:col-span-2">
                    ...
                    <?php if (!empty($user['stripe_customer_id']) && ($user['subscription_status'] == 'active' || $user['subscription_status'] == 'pending_cancellation')): ?>
                        
                        <?php if ($user['subscription_status'] == 'pending_cancellation'): ?>
                            <p class="text-yellow-300 mb-4">
                                Il tuo abbonamento è impostato per essere annullato alla fine del periodo di fatturazione. Puoi riattivarlo dal portale clienti.
                            </p>
                        <?php else: ?>
                    ...
                    <?php elseif ($user['subscription_status'] == 'lifetime' || $user['subscription_status'] == 'active'): ?>
                        <!-- Mostra questo se l'utente è Pro ma senza un ID Stripe (attivato manually) -->
