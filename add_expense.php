...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    try {
        ...
        if ($paid_by_user_id == $user_id) {
            if (empty($account_id)) {
                throw new Exception("Il campo 'Conto Personale' è obbligatorio quando paghi tu.");
            }
        }
        if (empty($fund_id) || empty($description) || $amount <= 0 || empty($expense_date) || empty($paid_by_user_id)) {
        ...
        switch ($split_method) {
            case 'equal':
                ...
                foreach ($split_with_users as $uid) { $splits[$uid] = $split_amount; }
                if ($remainder != 0 && !empty($split_with_users)) { $splits[$split_with_users[0]] += $remainder; }
                break;
            case 'fixed':
                $fixed_amounts = $_POST['fixed'] ?? [];
                foreach ($fixed_amounts as $uid => $val) {
                    if ((float)$val > 0) {
                        if (!in_array($uid, $member_ids)) throw new Exception("Utente non valido nella divisione.");
                        $splits[$uid] = (float)$val;
                        $total_check += (float)$val;
                    }
                }
                if (abs($total_check - $amount) > 0.015) { throw new Exception("La somma degli importi fissi non corrisponde al totale."); }
                break;
            case 'percentage':
                ...
                foreach ($percentages as $uid => $perc) {
                    if ((float)$perc > 0) {
                        if (!in_array($uid, $member_ids)) throw new Exception("Utente non valido nella divisione.");
                        $splits[$uid] = ($amount * (float)$perc) / 100;
                        $total_percentage += (float)$perc;
                    }
                }
                if (abs($total_percentage - 100) > 0.1) { throw new Exception("La somma delle percentuali non è 100%."); }
                $total_check = 0;
                foreach($splits as $uid => $val) { $splits[$uid] = round($val, 2); $total_check += $splits[$uid]; }
                $remainder = $amount - $total_check;
                if ($remainder != 0 && count($splits) > 0) { $splits[array_key_first($splits)] += $remainder; }
                break;
            default:
        ...
        if ($paid_by_user_id == $user_id) {
            ...
            $creator = get_user_by_id($conn, $user_id);
            $payer = get_user_by_id($conn, $paid_by_user_id);
            ...
            $message_text = "{$creator['username']} ha registrato una spesa di €" . number_format($amount, 2) . " a tuo nome nel fondo '{$fund['name']}'.";

            $sql = "INSERT INTO notifications (user_id, type, message, related_id) VALUES (?, 'expense_approval', ?, ?)";
            $stmt = $conn->prepare($sql);
            $stmt->bind_param("isi", $paid_by_user_id, $notification_data, $fund_id);
            $stmt->execute();
            $stmt->close();
            
            $message = "Richiesta di approvazione inviata a {$payer['username']}.";
        }

        $conn->commit();
        header("location: fund_details.php?id=" . $fund_id . "&message=" . urlencode($message) . "&type=success");
